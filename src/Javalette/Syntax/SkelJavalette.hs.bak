module Javalette.Syntax.SkelJavalette where

-- Haskell module generated by the BNF converter

import Javalette.Syntax.AbsJavalette
import Javalette.Syntax.ErrM
type Result = Err String

failure :: Show a => a -> Result
failure x = Bad $ "Undefined case: " ++ show x

transIdent :: Ident -> Result
transIdent x = case x of
  Ident string -> failure x
transProg :: Prog -> Result
transProg x = case x of
  Program topdefs -> failure x
transTopDef :: TopDef -> Result
transTopDef x = case x of
  FnDef type_ ident args blk -> failure x
transArg :: Arg -> Result
transArg x = case x of
  Argument type_ ident -> failure x
transBlk :: Blk -> Result
transBlk x = case x of
  Block stmts -> failure x
transStmt :: Stmt -> Result
transStmt x = case x of
  Empty -> failure x
  BStmt blk -> failure x
  Decl type_ items -> failure x
  Ass lvalue expr -> failure x
  Incr ident -> failure x
  Decr ident -> failure x
  Ret expr -> failure x
  VRet -> failure x
  Cond expr stmt -> failure x
  CondElse expr stmt1 stmt2 -> failure x
  While expr stmt -> failure x
  For type_ ident expr stmt -> failure x
  SExp expr -> failure x
transItem :: Item -> Result
transItem x = case x of
  NoInit ident -> failure x
  Init ident expr -> failure x
  InitObj ident constructor -> failure x
transConstructor :: Constructor -> Result
transConstructor x = case x of
  ArrayCon type_ expr -> failure x
transLValue :: LValue -> Result
transLValue x = case x of
  LIdent ident -> failure x
  LIndexed ident index -> failure x
transIndex :: Index -> Result
transIndex x = case x of
  Indx expr -> failure x
transType :: Type -> Result
transType x = case x of
  Int -> failure x
  Doub -> failure x
  Bool -> failure x
  Void -> failure x
  Array type_ -> failure x
  String -> failure x
  Fun type_ types -> failure x
transExpr :: Expr -> Result
transExpr x = case x of
  EVar ident -> failure x
  ELitInt integer -> failure x
  ELitDoub double -> failure x
  ELitTrue -> failure x
  ELitFalse -> failure x
  EApp ident exprs -> failure x
  EString string -> failure x
  Neg expr -> failure x
  Not expr -> failure x
  Dot expr ident -> failure x
  EIndex expr index -> failure x
  EMul expr1 mulop expr2 -> failure x
  EAdd expr1 addop expr2 -> failure x
  ERel expr1 relop expr2 -> failure x
  EAnd expr1 expr2 -> failure x
  EOr expr1 expr2 -> failure x
  EAnn type_ expr -> failure x
transAddOp :: AddOp -> Result
transAddOp x = case x of
  Plus -> failure x
  Minus -> failure x
transMulOp :: MulOp -> Result
transMulOp x = case x of
  Times -> failure x
  Div -> failure x
  Mod -> failure x
transRelOp :: RelOp -> Result
transRelOp x = case x of
  LTH -> failure x
  LE -> failure x
  GTH -> failure x
  GE -> failure x
  EQU -> failure x
  NE -> failure x

